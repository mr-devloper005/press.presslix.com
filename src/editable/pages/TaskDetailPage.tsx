...
export function TaskDetailView({ task, post, related, comments = [] }: { task: TaskKey; post: SitePost; related: SitePost[]; comments?: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  const detailVars = { '--detail-bg': '#ffffff', '--detail-text': '#141a26', '--detail-surface': '#f8fbff', '--detail-accent': '#2854c5' } as CSSProperties
...
function ArticleDetail({ task, post, related, comments }: { task: TaskKey; post: SitePost; related: SitePost[]; comments: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  ...
  return (
    <section className="bg-white">
      <header className="border-b border-[#e5e9f2]">
        <div className="mx-auto max-w-[1180px] px-4 py-8 sm:px-6 lg:px-8 lg:py-12">
          <BackLink task={task} />
          <div className="mt-10 flex flex-wrap items-center justify-between gap-3 border-t-4 border-[var(--slot4-accent)] pt-4 text-[11px] font-black uppercase tracking-[0.16em]">
            <span className="text-[var(--slot4-accent)]">{categoryOf(post, 'News')}</span>
            {published ? <time>{published}</time> : null}
          </div>
          <h1 className="mt-6 max-w-6xl text-4xl font-black leading-tight text-[var(--slot4-accent)] sm:text-6xl lg:text-[4.6rem]">{post.title}</h1>
          {summaryText(post) ? <p className="mt-6 max-w-4xl text-xl font-bold leading-8 text-[#4a5568] sm:text-2xl">{summaryText(post)}</p> : null}
        </div>
      </header>

      {images[0] ? (
        <figure className="mx-auto max-w-[1180px] overflow-hidden rounded-b-lg border-x border-b border-[#e5e9f2] bg-white shadow-sm">
          <img src={images[0]} alt="" className="max-h-[620px] w-full object-cover" />
          <figcaption className="border-t border-[#e5e9f2] px-4 py-3 text-xs text-black/55 sm:px-6">Featured image for {post.title}</figcaption>
        </figure>
      ) : null}

      <div className="mx-auto grid max-w-[1180px] gap-12 px-4 py-12 sm:px-6 lg:grid-cols-[minmax(0,760px)_300px] lg:px-8 lg:py-16">
        <article className="min-w-0 border-t-4 border-[var(--slot4-accent)] pt-8">
...
function RelatedPanel({ task, post, related, compact = false }: { task: TaskKey; post: SitePost; related: SitePost[]; compact?: boolean }) {
  ...
  return (
    <aside className="min-w-0 space-y-5">
      {!compact ? (
        <div className="rounded-lg border border-[#e5e9f2] bg-white p-5 shadow-sm">
...
function RelatedCard({ task, post }: { task: TaskKey; post: SitePost }) {
  ...
  return (
    <Link href={buildPostUrl(task, post.slug)} className="group flex gap-3 border-t border-[#e5e9f2] py-3 transition hover:text-[var(--slot4-accent)]">
      {image && task !== 'sbm' ? <img src={image} alt="" className="h-20 w-20 shrink-0 rounded-md object-cover" /> : <div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-md bg-[var(--slot4-accent)] text-white"><FileText className="h-6 w-6" /></div>}
...
function EditableComments({ slug, comments }: { slug: string; comments: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  return (
    <section className="mt-12 rounded-lg border border-[#e5e9f2] border-t-4 border-t-[var(--slot4-accent)] bg-white p-5 shadow-sm">
