      ...
      <main className="min-h-screen bg-white text-[#141a26]">
        <section className="border-b border-[#e5e9f2] bg-white">
          <div className="mx-auto flex max-w-[var(--editable-container)] flex-col gap-6 px-4 py-10 sm:px-6 lg:flex-row lg:items-end lg:justify-between lg:px-8 lg:py-14">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.28em] text-[var(--slot4-accent)]">The newsroom</p>
              <h1 className="mt-3 text-5xl font-black leading-tight text-[var(--slot4-accent)] sm:text-6xl lg:text-7xl">
                {category === 'all' ? label : categoryLabel}
              </h1>
            </div>
            <p className="max-w-md border-l-4 border-[var(--slot4-accent)] pl-5 text-sm font-bold leading-7 text-[#667085]">
              Timely announcements, media-ready updates, and clean distribution pages organized for fast discovery.
            </p>
          </div>
        </section>

        <section className="border-b border-[#dfe5f0] bg-[var(--slot4-accent)] text-white">
          <div className="mx-auto flex max-w-[var(--editable-container)] gap-7 overflow-x-auto px-4 py-4 text-xs font-black uppercase tracking-[0.16em] sm:px-6 lg:px-8">
            <Link href={basePath} className={category === 'all' ? 'text-white underline underline-offset-4' : 'hover:text-white/75'}>Latest</Link>
            {categories.slice(0, 8).map((item) => (
              <Link key={item.slug} href={pageHref(basePath, item.slug, 1)} className={category === item.slug ? 'text-white underline underline-offset-4' : 'whitespace-nowrap hover:text-white/75'}>
                {item.name}
              </Link>
            ))}
          </div>
        ...
        {lead ? (
          <section className="mx-auto grid max-w-[var(--editable-container)] border-x border-[#e5e9f2] bg-white lg:grid-cols-[1.75fr_0.75fr]">
            <Link href={`${basePath}/${lead.slug}`} className="group relative min-h-[34rem] overflow-hidden border-b border-[#e5e9f2] lg:border-b-0 lg:border-r">
            ...
            <div className="grid">
              <div className="border-b border-[#e5e9f2] bg-[var(--slot4-accent)] p-6 text-white">
                <p className="text-xs font-black uppercase tracking-[0.24em]">Top stories</p>
                <p className="mt-3 text-3xl font-black leading-tight">What the newsroom is watching now.</p>
              </div>
              {secondary.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group grid grid-cols-[7rem_1fr] border-b border-[#e5e9f2] bg-white last:border-b-0">
                  <img src={getImage(post)} alt="" className="h-full min-h-40 w-full object-cover grayscale transition group-hover:grayscale-0" />
                  <div className="p-5">
                    <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[var(--slot4-accent)]">0{index + 1}</p>
        ...
        <section className="mx-auto max-w-[var(--editable-container)] border-x border-[#e5e9f2] bg-[#f8fbff] px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
          <div className="mb-8 flex flex-wrap items-end justify-between gap-5 border-b-4 border-[var(--slot4-accent)] pb-4">
            <h2 className="text-4xl font-black text-[var(--slot4-accent)] sm:text-5xl">More from the desk</h2>
            <form action={basePath} className="flex rounded-md border border-[#d8deea] bg-white">
              ...
              <button className="h-11 bg-[var(--slot4-accent)] px-5 text-xs font-black uppercase tracking-[0.14em] text-white">Filter</button>
            </form>
          </div>
          ...
          {remaining.length ? (
            <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
              {remaining.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group overflow-hidden rounded-lg border border-[#e5e9f2] bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
                  <div className="aspect-[16/10] overflow-hidden bg-[#eef3ff]">
                  ...
                  <div className="p-5">
                    <div className="flex items-center justify-between gap-4 text-[10px] font-black uppercase tracking-[0.18em] text-[var(--slot4-accent)]">
                      <span className="text-[var(--slot4-accent)]">{getCategory(post, label)}</span><span>{String(index + 3).padStart(2, '0')}</span>
                    </div>
            ...
            <div className="rounded-lg border border-dashed border-[#d8deea] bg-white p-12 text-center">
          ...
          <div className="mt-10 flex items-center justify-center gap-0">
            {pagination.hasPrevPage ? <Link href={pageHref(basePath, category, page - 1)} className="rounded-l-md border border-[#d8deea] bg-white px-5 py-3 text-xs font-black uppercase">Previous</Link> : null}
            <span className="border-y border-[#d8deea] bg-[var(--slot4-accent)] px-5 py-3 text-xs font-black uppercase text-white">Page {page} / {pagination.totalPages || 1}</span>
            {pagination.hasNextPage ? <Link href={pageHref(basePath, category, page + 1)} className="rounded-r-md border border-[#d8deea] bg-white px-5 py-3 text-xs font-black uppercase">Next</Link> : null}
          </div>
        </section>
      </main>
    </EditableSiteShell>
